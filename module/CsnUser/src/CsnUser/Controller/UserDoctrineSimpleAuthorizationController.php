...
class UserDoctrineSimpleAuthorizationController extends AbstractActionController
{
	private $usersTable;
	// for Doctrine
...
//-		$this->registerDocrineClassLoader();		
	...
	// -------------------------------------------------
		// I) Authorization
		// 1) Every time new instance of AuthenticationService
		//$auth = new \Zend\Authentication\AuthenticationService();
		// or
		// you have to add code to global.php and then 
		// 2) With Service Manager 
		$auth = $this->getServiceLocator()->get('Zend\Authentication\AuthenticationService');
		// or
		// 3) With a controller plugin
		...
		//	}
         // someone is logged !
		echo '<h1>hasIdentity = ' . $auth->hasIdentity() . '</h1>'; // at this moment the SESSIOn works again
		if (!$auth->hasIdentity()) return $this->redirect()->toRoute('home', array('controller' => 'index', 'action' => 'login'));
		$user = $auth->getIdentity();

//-		$user = $this->identity(); // Using Controller Plugin. Don't forget to set the service in global.php Zend\Authentication\AuthenticationService
...
//-		echo '</pre>';
		...
		if ($usrlId < 2) return $this->redirect()->toRoute('home', array('controller' => 'index', 'action' => 'login'));
	// -------------------------------------------------
...
//-		$users = $entityManager->find('CsnUser\Entity\User', 2);
	...
	public function createAction()
	{
		...
		$auth = $this->getServiceLocator()->get('Zend\Authentication\AuthenticationService');
		if (!$auth->hasIdentity()) return $this->redirect()->toRoute('auth/default', array('controller' => 'index', 'action' => 'login'));
		$user = $auth->getIdentity();
		$usrlId = $user->getUsrlId();	
		// II) Protect our action
		if ($usrlId < 2) return $this->redirect()->toRoute('auth/default', array('controller' => 'index', 'action' => 'login'));		
...
//				$this->getUsersTable()->insert($data);
	...
	public function updateAction()
	{
		...
		$usrlId = $user->getUsrlId();			
		// II) Protect our action
		if ($usrlId < 3) return $this->redirect()->toRoute('auth/default', array('controller' => 'index', 'action' => 'login'));		
		//------------------------- End the block for Authorization
	
		$id = $this->params()->fromRoute('id');
		if (!$id) return $this->redirect()->toRoute('csn_user/default', array('controller' => 'user-doctrine-simple-authorization', 'action' => 'index'));
