...
class UserDoctrineSimpleAuthorizationAclController extends AbstractActionController
{
	private $usersTable;
	// for Doctrine
		...
		if ($auth->hasIdentity()) {
			...
			switch ($usrl_id) {
				...
				case 2 :
					...
					break;
				default :
					$role = \CsnUser\Acl\Acl::DEFAULT_ROLE; // guest
					break;
			}
		}
...
//				$this->getUsersTable()->insert($data);
	...
	public function updateAction()
	{
		...
		if (!$id) return $this->redirect()->toRoute('csn_user/default', array('controller' => 'user-doctrine-simple-authorization-acl', 'action' => 'index'));
